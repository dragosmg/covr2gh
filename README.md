...
On a GitHub pull request, {covr2gh} runs `covr::package_coverage()` on
both branches involved. It compares the outputs, summarises key
findings, and posts them in a comment.
...
It comes with a helper function that generates a GitHub action workflow
and adds a badge to your README, if it can find the {usethis} badge
“block” (`<!-- badges: start -->` and `<!-- badges: end -->`).
